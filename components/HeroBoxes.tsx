...
function ProductTile({ item, mobile = false }: { item: (typeof boxTypes)[number]; mobile?: boolean }) {
  return <Link href={`/boxes/${item.slug}`} className={`card focus-ring group shrink-0 p-3 transition-colors hover:border-ink ${mobile ? "w-[190px]" : ""}`}>
    <div className="relative aspect-[4/3] overflow-hidden rounded bg-white"><Image src={item.image} alt={item.name} fill sizes={mobile ? "190px" : "(max-width: 1024px) 25vw, 300px"} className="object-contain p-2" /></div>
...
export function HeroBoxes() {
  ...
  return <div className="mt-14 border-t border-line pt-10">
